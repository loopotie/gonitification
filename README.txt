...
- Corner labels merge at shared endpoints, and their positions avoid nearby edges and other labels. Side measurements follow the edge direction while staying upright; labels beside near-vertical edges stay horizontal. Rhombuses are slanted parallelograms with equal sides, and every circle shows its center point.
- Canvas capacity sets the maximum span to 100, 500, 1,000, or 10,000 units. Changing capacity preserves existing measurements. Use Zoom and Pan canvas to work with the selected scale.
- Auto draw can create circles, rectangles, squares, parallelograms, rhombuses, triangle types, and regular polygons with 5–10 sides. Enter any sufficient combination of dimensions; unused values can stay blank. It derives the other dimensions and a measurement summary, and rejects conflicting numeric measurements. If you enter a variable such as x, it creates a clearly marked schematic that is not to scale and reports only the values you supplied. Numeric values that do not determine a shape show an error instead of inventing dimensions.
- Hand-drawn dimensions and areas are calculated from the vector shape. The board flags mismatches between typed side or angle labels and the drawn proportions; check the sketch and values before relying on them mathematically.
